...
- util-1 뉴스 분류 저장은 네이버 응답에서 생성한 `NewsSourceRecord`를 원문·출처·시각의 기준으로 사용한다.
- AI는 위험 묶음만 생성하며, `risk_groups[].source_ids`가 이번 입력의 구조화 뉴스 ID를 하나 이상 참조하지 않으면 저장하지 않는다.
- AI가 반환한 `source_items`로 원본 뉴스 메타데이터를 덮어쓰지 않는다. 운영 전에는 이 구조화 원문 저장을 전용 DB와 원문 사용권 정책으로 교체해야 한다.
- 수동 테스트 저장과 자동 뉴스 분류 저장은 기능 1번 내부에서만 분리한다. 수동 기록은 위험 묶음에 포함하지 않고, 자동 분류만 `news-classifications.json`에 누적한다.
