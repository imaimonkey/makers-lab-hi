...
export function deriveArticleDashboard(records: ArticleSourceRecord[]): {
  ...
  const news = records.map((record) => {
    const privateArticleFields = new Set(['text', 'fileName', 'fileUrl', 'sourcePath', 'format', 'contentProfile', 'derived'])
    const article = Object.fromEntries(Object.entries(record).filter(([key]) => !privateArticleFields.has(key))) as RadarNewsArticle
