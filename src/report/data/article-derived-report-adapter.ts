...
const ARTICLE_CRITERIA: Array<[string, string, string]> = [
  ['actual_market_demand', 'market', '실제 시장 수요'],
  ['risk_pooling', 'insurability', '위험 분산 가능성'],
  ['fortuity', 'insurability', '우연성'],
  ['insurable_interest', 'insurability', '피보험이익'],
  ['moral_hazard_control', 'insurability', '고의·도덕적 해이 통제'],
  ['gambling_like_structure', 'insurability', '사행성 배제·실손보상 원칙'],
  ['loss_verifiability', 'coverage', '손해 확인·산정 가능성'],
  ['pml_accumulation', 'coverage', '최대가능손해(PML)'],
  ['liability_clarity', 'coverage', '책임주체 명확성'],
  ['wording_clarity', 'wording', '약관·보장조건 명확성'],
  ['pricing_data_readiness', 'data', '위험 데이터 확보 가능성'],
  ['coverage_gap', 'coverage', '보장 공백·중복 여부'],
]

function articleCriterionCopy(article: ArticleSourceRecord, criterionId: string) {
  ...
  const metricSummary = derived.metrics.slice(0, 3).map((metric) => `${metric.label}: ${metric.value}`).join(' · ')
  const byId: Record<string, { summary: string; rationale: string }> = {
    actual_market_demand: { summary: derived.summary, rationale: `${targets}를 중심으로 문서에서 확인된 수요·성장 신호를 연결했습니다. ${facts}` },
    risk_pooling: { summary: `${targets}의 노출을 위험군·지역·계약 단위로 나누어 관리할 수 있는지 검토합니다.`, rationale: `문서에서 제시한 영향 대상과 산업 범위를 기준으로 위험 집단을 나눌 수 있는지 확인합니다. ${targets}` },
    fortuity: { summary: derived.event, rationale: `사고 발생 여부와 시점이 사전에 확정되는지, 문서에 제시된 사건 구조가 우연한 손해인지 검토합니다. ${derived.changeType}` },
    insurable_interest: { summary: `${targets}의 경제적 손해와 보험금 수령 주체의 연결을 검토합니다.`, rationale: `문서에서 확인된 피해 대상과 책임 후보를 보험목적·손해 부담 주체와 연결합니다. ${targets}` },
    moral_hazard_control: { summary: derived.uncertainty[0] ?? '사고 원인·손해 입증과 고의·과다청구 통제 기준을 검토합니다.', rationale: `문서에 포함된 손해 유형을 객관적 자료로 확인하고 고의·과다청구 가능성을 분리해야 합니다. ${damages}` },
    gambling_like_structure: { summary: `${damages}와 같이 실제 손해가 확인되는 위험을 기준으로 보상 구조를 검토합니다.`, rationale: `실제 경제적 손해와 문서상 위험 사건을 연결해 정액·투자성 구조가 아닌지 확인합니다. ${derived.event}` },
    loss_verifiability: { summary: `${damages}의 발생 여부와 손해 규모를 객관적으로 확인할 수 있는지 검토합니다.`, rationale: `문서의 사실·지표를 손해자료와 연결할 수 있는지 확인합니다. ${facts}` },
    pml_accumulation: { summary: `${damages}가 한 사고 또는 다수 목적물에 동시에 발생할 때의 최대 손해를 검토합니다.`, rationale: `노출 대상과 손해 유형을 기준으로 사고당 손해와 누적손해를 분리해야 합니다. ${targets} · ${damages}` },
    liability_clarity: { summary: `${targets} 사이의 책임 주체와 책임 분담 기준을 구분합니다.`, rationale: `문서에 제시된 이해관계자별 역할·책임을 사고 원인, 운영, 손해 보유 주체로 나누어 확인합니다.` },
    wording_clarity: { summary: `${derived.event}와 ${damages}를 보장사고·보상손해·면책으로 구분할 수 있는지 검토합니다.`, rationale: `문서의 위험 표현을 약관상 사고 정의와 지급요건으로 변환하고 불명확한 범위를 별도 기록합니다.` },
    pricing_data_readiness: { summary: metricSummary || '문서에서 확인된 정량 지표와 손해자료 연결 수준을 검토합니다.', rationale: `문서에 포함된 지표를 사고 빈도·손해액·노출량 자료로 전환할 수 있는지 확인합니다. ${metricSummary}` },
    coverage_gap: { summary: derived.coverageGap, rationale: `기존 보장과 문서에서 확인된 손해 유형 사이의 중복·공백을 구분합니다. ${damages}` },
  }
  return byId[criterionId] ?? { summary: derived.summary, rationale: facts }
}

function articleCriteria(article: ArticleSourceRecord, evidenceIds: string[]) {
  return ARTICLE_CRITERIA.map(([id, category, title], index) => {
    ...
    return {
      ...
      question: `${title}을(를) 문서 근거와 실제 운영자료로 확인할 수 있는가?`,
      ...
      analysisDetail: {
        ...
        assumptions: ['문서에 포함된 사실·지표를 상품화 후보 검토용으로 연결'],
