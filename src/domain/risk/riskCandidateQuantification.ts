...
// First-pass sample estimate for the exploration screen when a candidate lacks a confirmed PML amount.
...
function prototypePmlMetric(record: RiskExplorationRecord): RiskCandidateQuantificationMetric {
  ...
  return {
    ...
    result: `약 ${result}억 원 · 상품화 종합평가 산정식 기반`,
    numericValue: result,
    unit: '억원',
    confidence: '기사 기반',
...
export function getRiskCandidateQuantification(record: RiskExplorationRecord): RiskCandidateQuantification {
  ...
  const marketScore = articleScore ? Math.round(articleScore.market * 20) : reference?.marketScore ?? Math.round(record.metricScores.demand * 20)
  ...
  return {
    ...
    pml: articleScore ? {
      ...prototypePmlMetric(record),
      sub: 'PML 기준',
      official: record.metricEvidence?.accumulation?.quotes ?? [],
      assumption: ['심각성·누적성·보장 공백 지표를 기준으로 탐색용 금액을 환산', '실제 손해액·누적 범위 연결 전 재산정 필요'],
      formula: ['PML = 단일사고 손해 + 동시다발 누적손해 + 시설·긴급대응 비용'],
      result: `${prototypePmlMetric(record).value} · 누적도·사고 가능성 연동 대략 산정`,
      confidence: '확인 필요',
      evidenceIds: record.evidenceIds ?? [],
      uncertainty: ['실제 손해액·누적 범위가 연결되면 재산정'],
    } : hasPrototypeReference ? {
      key: 'pml', label: 'PML', value: reference?.pml ?? '확인 필요', sub: 'PML 기준', color: 'red',
