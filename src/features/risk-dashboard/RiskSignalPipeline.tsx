...
export function RiskSignalPipeline({ radarSnapshot }: { radarSnapshot: RiskRadarSnapshotState }) {
  ...
  const isDashboardLive = radarSnapshot.sourceStatus.dashboard === 'live'
  const pipelineMetrics = {
    unresolved: radarSnapshot.dashboard.metrics.totalSignals ?? radarSnapshot.dashboard.metrics.news ?? demoDashboardMetrics.unresolved,
    candidates: radarSnapshot.risks.length,
    gaps: demoDashboardMetrics.gaps,
    lawPending: radarSnapshot.dashboard.metrics.verificationPending ?? radarSnapshot.dashboard.metrics.evidencePending,
    reviewerPending: radarSnapshot.dashboard.metrics.reviewerPending ?? demoDashboardMetrics.reviewerPending,
  }
    ...
    <section className="risk-signal-pipeline" aria-label="위험 레이더 통합 신호 파이프라인">
      <div className="risk-pipeline-ribbon surface-card">
        ...
        <div className="risk-pipeline-metrics">
          <span><strong>{pipelineMetrics.unresolved}</strong>전체 신호</span>
          <span><strong>{pipelineMetrics.candidates}</strong>후보</span>
          <span><strong>{pipelineMetrics.gaps}</strong>보장 공백 SAMPLE</span>
          <span><strong>{pipelineMetrics.lawPending}</strong>법령 보류</span>
        </div>
      </div>
      ...
      <div className="risk-pipeline-grid">
        ...
        <article className="risk-pipeline-flow surface-card">
          <div className="panel-heading"><div><p className="eyebrow">PROCESS GATES</p><h2>검증 단계 현황</h2></div><span className="updated-label">{isDashboardLive ? 'LIVE API' : 'SAMPLE'}</span></div>
          <ol className="risk-pipeline-steps">
            <li><span>01</span><div><strong>수집</strong><small>뉴스·현장 신호</small></div><b>{pipelineMetrics.unresolved}</b></li>
            <li><span>02</span><div><strong>위험 후보화</strong><small>클러스터·반복성</small></div><b>{pipelineMetrics.candidates}</b></li>
            <li><span>03</span><div><strong>근거·법령 확인</strong><small>검증 보류 포함</small></div><b>{pipelineMetrics.lawPending}</b></li>
            <li><span>04</span><div><strong>상품성 검토</strong><small>담당자 판단</small></div><b>{pipelineMetrics.reviewerPending}</b></li>
          </ol>
