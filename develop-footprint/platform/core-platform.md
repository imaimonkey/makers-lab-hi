...
| `/sales-intake` | `SalesIntakePage` | 영업부서 현장 리포트 |

`RootLayout`은 실무자 전용 사이드바, 상단 사용자 맥락, 비공식 프로토타입 고지를 제공한다. `CustomerLayout`은 공개 웹 형태의 헤더, 고객용 탐색 내비게이션, 별도 푸터를 제공한다. `SalesLayout`은 보험상품개발 워크벤치와 같은 내부 테마로 영업부서 현장 리포트 입력과 검토 대기 흐름을 제공한다. 세 셸은 `ModeSwitch`로 전환하지만 고객·영업 앱은 실무자 좌측 메뉴에 포함하지 않는다.
...
현재 고객·영업 입력과 화면 필터는 React 로컬 상태다. 비식별 고객 신호는 `customerSignalStorage.ts`, 영업부서 리포트는 `src/domain/sales/salesIntake.ts` 어댑터를 통해 `localStorage`에 저장한다. 새 전역 상태 라이브러리를 추가하기 전에 서버 상태, URL 상태, 폼 상태, 세션 상태 중 무엇인지 먼저 분류한다.
