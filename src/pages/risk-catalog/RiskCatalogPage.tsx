...
export function RiskCatalogPage({ mode = 'analyst' }: { mode?: 'analyst' | 'developer' }) {
  ...
  return (
    <div className="page catalog-page seoyeon-visual">
      ...
      />
      <div className="sample-notice"><span>{mode === 'developer' ? 'CONTENT-DERIVED SAMPLE' : 'SAMPLE'}</span>{mode === 'developer' ? <><span>src/article 본문을 읽어 구조화 결과 형태로 표시합니다. 저장된 실제 Step 2 결과가 있으면 우선 사용하며, 버튼으로 실제 분석 연결을 별도 실행할 수 있습니다.</span><button type="button" onClick={() => void runDeveloperStep2()} disabled={developerRunning}>{developerRunning ? '분석 중...' : '실제 Step 2 전체 실행 · Excel 저장'}</button>{developerMessage ? <small role="status">{developerMessage}</small> : null}</> : sampleOnlyNotice}</div>
