...
function RiskDetailLink({ risk, label, developerMode = false }: { risk: ProductRisk; label: string; developerMode?: boolean }) {
  ...
  return workbenchRiskId ? <Link to={`${isDeveloper ? '/developer-test' : ''}/risks/${workbenchRiskId}`}>{label}</Link> : <span className="table-empty">{label} · 상세 대기</span>
}
  ...
  async function runAction<T>(key: string, action: () => Promise<T>, success: string, afterSuccess?: (result: T) => void) {
    ...
    try {
      ...
      const refreshNotice = refreshResult.failedSources.length
        ? ` 일부 재조회 실패(${refreshResult.failedSources.join(', ')}), 기존 기본값·이전 응답을 유지합니다.`
        : ' news·risks·issues를 다시 조회했습니다.'
      setNotice(`${success}${refreshNotice}`)
      afterSuccess?.(result)
    } catch {
      setNotice('운영 API가 연결되지 않아 기본 데이터는 변경하지 않았습니다. 작업 경계와 실패 상태만 확인할 수 있습니다.')
    } finally {
      setBusy('')
    }
  }
  ...
  return (
    <section className="risk-exploration-operations surface-card" aria-labelledby="risk-operations-title">
      ...
      {developerMode ? <div className="risk-operations-summary" aria-label="실제 아티클 및 Step 2 상태">
        ...
        <article><span>DATA STATUS</span><strong>LOCAL ACTUAL</strong><small>src/article PDF · Excel Step 2 결과</small></article>
        ...
        <article><span>LAST RESPONSE</span><strong>{catalogSnapshot.lastSuccessfulAt ? 'OK' : '—'}</strong><small>{catalogSnapshot.refreshing ? '재조회 중' : 'API 미확인 시 기본값'}</small></article>
      </div>}
      {!developerMode && catalogSnapshot.initialLoading ? <p className="risk-operations-notice" role="status">뉴스·위험 후보·이슈를 불러오는 중입니다. 기존 목록을 유지합니다.</p> : null}
      {!developerMode && Object.keys(catalogSnapshot.errors).length ? <p className="risk-operations-notice" role="status">일부 API 응답이 없어 실패한 원천은 기본값 또는 이전 응답으로 유지됩니다.</p> : null}
      ...
      <div className="risk-operations-summary"><article><span>탐색 후보</span><strong>{risks.length}</strong><small>현재 필터</small></article><article><span>본문 확보</span><strong>{developerMode ? (developerData?.counts.bodyReady ?? 0) : 31}</strong><small>{developerMode ? 'src/article' : period}</small></article><article><span>기업 수요</span><strong>{developerMode ? (developerData?.counts.corporateDemand ?? 0) : 14}</strong><small>{developerMode ? 'Step 2 후보 결과' : source}</small></article><article><span>법령 변화</span><strong>{developerMode ? (developerData?.counts.laws ?? 0) : demoLaws.length}</strong><small>{developerMode ? 'Step 2 law 결과' : `${region} · 확인 대기`}</small></article></div>

      <div className="risk-operations-context-grid">
        <article><div className="risk-operations-section-heading"><div><p className="eyebrow">EMERGING KEYWORDS</p><h3>주요 키워드</h3></div><span>{developerMode ? 'ACTUAL ARTICLE · STEP 2' : '탐색 기준'}</span></div><div className="risk-operations-cloud">{activeKeywords.map((keyword, index) => <button type="button" className={`weight-${index % 4}`} key={keyword} onClick={() => setQuery(keyword)}>{keyword}</button>)}</div></article>
