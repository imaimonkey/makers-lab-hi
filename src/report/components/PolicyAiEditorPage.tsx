...
export function PolicyAiEditorPage({
  ...
  useEffect(() => {
    const loadSelectedEntry = async () => {
      if (embeddedReport && embeddedRiskData) {
        ...
        setSavedEditor(selected.report.wordingFeasibility.policyEditor)
  ...
  const handleAction = (action: Message['action']) => {
    if (action === 'draft') {
      ...
      const existsInReport = reportClauses.some((c) => isSameClause(c, generatedClause))
      const existsInAdded = addedClauses.some((c) => isSameClause(c, generatedClause))
      if (existsInAdded) {
        respond('동일한 조항이 이미 추가되어 있습니다. 중복 추가하지 않았습니다.')
        return
      }
      if (existsInReport) {
        respond('기존 약관에 동일한 조항이 있어 추가하지 않았습니다.')
        return
      }
      setAddedClauses((current) => dedupeClauses([...current, generatedClause]))
      setStatus('editing')
  ...
  const save = async () => {
    ...
    const nextEditor: WordingPolicyEditor = {
      articleDrafts: savedEditor?.articleDrafts ?? {},
