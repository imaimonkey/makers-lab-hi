...
- 최상단 상품개발 제안 카드는 상세 상품정보표가 아니라 `workingName`, 한 문장 설명, `expectedPolicyholder`, 상품 형태, 보장 범위의 핵심 요약으로 구성한다. 보험 대상·보험 영역·보험 종목·가입 대상·주요 계약자를 반복해서 노출하지 않는다.
- `expectedPolicyholder`는 계약자 후보로, `expectedInsured`와 `coveredObject`는 보장 대상 후보로 구분한다. 계약자와 피보험자를 같은 역할로 단정하지 않는다.
- 추천 이유는 제목과 짧은 bullet 중심으로 작성하고, 체크 아이콘·반복 배지·중첩 카드 없이 핵심 판단을 우선한다.
- 보험금 지급 흐름, 보장 구조 요약, 계약 및 보호 관계는 같은 `productProposal`의 계약·보장·지급 방향을 사용한다. 영향 방향은 문장 안에 통합하고 상태 배지를 남발하지 않는다.
- `coverageLimitDirection`은 상품 조건의 제안 근거만 작성한다. PML 자체의 Low/Base/High 값과 산정식은 상품화 종합평가의 공통 PML 원본을 참조하며, 이 탭에서 새로 계산하거나 다른 값을 하드코딩하지 않는다.
- `settlementDirection`에는 기존 보험 적용·지급액 차감·선지급·구상 검토 방향을 구분해 쓴다. 확정된 지급 약속이나 법률상 책임을 단정하지 않는다.
