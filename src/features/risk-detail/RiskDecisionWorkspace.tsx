  ...
  return (
    <section className="detail-workspace" aria-label="위험상세 평가 워크스페이스">
        ...
        <aside>
          ...
          <details className="score-logic-disclosure">
            ...
            <div className="logic-step-list">
              ...
              <p className="logic-step-note">현재 SAMPLE은 수요 신호를 priorityIndex로 직접 사용합니다. 아래 세 값은 assessmentScore를 다시 합산하지 않는 설명용 driverScore입니다.</p>
      ...
      <section className="detail-next-checks surface-card" aria-labelledby="next-checks-title">
        <div className="detail-next-checks-heading"><div><p className="eyebrow">NEXT REVIEW ACTIONS</p><h2 id="next-checks-title">다음 확인 항목</h2><p>결론 확정 전에 확인할 실무 항목입니다. 체크 상태는 이 브라우저의 SAMPLE 진행 상태로만 저장되며 서버·다른 사용자에게 반영되지 않습니다.{checksSavedAt ? ` 마지막 로컬 저장 ${new Date(checksSavedAt).toLocaleString('ko-KR')}` : ''}</p></div><strong>{checkedNextSteps.length}<small>/{nextSteps.length} 완료</small></strong></div>
      ...
      <div className="detail-workspace-grid" id="assessment-signal">
        <article className="detail-trend-panel surface-card">
           <div className="panel-heading">
             <div><p className="eyebrow">TREND INDEX · SAMPLE</p><h2>신호 추이</h2></div>
             <div className="panel-heading-side"><span className="updated-label">최근 8개월 · SAMPLE</span><details className="score-logic-disclosure score-logic-disclosure--light"><summary>trendIndex 계산</summary><div className="logic-step-list"><p><b>01</b><span>기준값</span><code>현재 수요 신호 = {risk.signalStrength}</code></p><p><b>02</b><span>월별 값</span><code>max(20, 기준값 − 보정값)</code></p>{trendMonths.map((month, index) => <p key={month}><b>{String(index + 3).padStart(2, '0')}</b><span>{month}</span><code>max(20, {risk.signalStrength} − {trendOffsets[index]}) = {trend[index]}</code></p>)}<p className="logic-step-note"><b>변화율</b> = ({risk.signalStrength} − 최저값 {minTrend}) ÷ {minTrend} × 100 = +{trendRise}%</p><p className="logic-step-note">{buildTrendAiSummary(risk, trendRise)}</p></div></details></div>
          </div>
          <p className="detail-panel-description">trendIndex · 절대 손실액·보험료가 아닌 TOP-10 관측 변화에서 파생한 정규화 SAMPLE 지수입니다. priorityIndex와 같은 값으로 해석하지 않습니다.</p>
          <div className="detail-sparkline-layout">
            <div>
                <div className="detail-sparkline" role="img" aria-label={`${risk.title} 최근 8개월 SAMPLE trendIndex 추이`}>
              ...
              {showTable ? <div className="detail-chart-table-wrap"><table><caption className="sr-only">최근 8개월 trendIndex SAMPLE 지수 표</caption><thead><tr>{trendMonths.map((month) => <th scope="col" key={month}>{month}</th>)}</tr></thead><tbody><tr>{trend.map((value, index) => <td key={trendMonths[index]}>{value}</td>)}</tr></tbody></table></div> : null}
            </div>
            <aside><strong>+{trendRise}%</strong><span>trendIndex 8개월 SAMPLE 변화</span><dl><div><dt>최고 관측</dt><dd>{risk.signalStrength}</dd></div><div><dt>관측 범위</dt><dd>비교 레코드</dd></div><div><dt>근거 유형</dt><dd>{evidenceTypes.length - 1}개</dd></div></dl></aside>
          </div>
        </article>
        ...
        <article className={showEvidence ? 'detail-evidence-panel surface-card is-expanded' : 'detail-evidence-panel surface-card is-collapsed'}>
          <div className="panel-heading">
            ...
            <span className="status-badge sample">{linkedSourceCount}/{evidenceLedger.length} URL 연결 · {evidenceLedger.length ? evidenceLedger[0].dataStatus : 'pending'}</span>
          </div>
