...
export function StandardRiskDetail({ risk, detail }: StandardRiskDetailProps) {
  ...
  const narrative = getRiskDetailNarrative(risk.id)
  ...
  return (
    <main className="ai-copyright-detail standard-risk-detail">
      ...
      <section className="aic-fact-grid">
        ...
        ].map(([label, title, body], index) => <article className="aic-fact" key={label}><small>{label}</small><strong>{title}</strong><p>{body}</p>{index === 0 && primarySource ? <a href={primarySource} target="_blank" rel="noreferrer noopener">{primarySourceName} ↗</a> : <span>SAMPLE · 확인 필요</span>}</article>)}
      </section>
      ...
      <div className="aic-layout">
        <div className="aic-content">
          ...
          <section className="aic-panel" id="aic-damage"><PanelTitle number="05" title="손해와 비용의 확산 경로" subtitle="예상 보험금이 아니라 실제로 발생 가능한 손해 유형을 구분합니다." /><div className="aic-damage-path">{lossRows.map(([title, body, tag]) => <div className="aic-damage-row" key={title}><strong>{title}</strong><p>{body}</p><span><b>{tag}</b>원문·약관 확인</span></div>)}</div><div className="aic-amount-note"><b>금액 표시 원칙:</b> 공개된 통계·판결·실제 비용이 원문에서 확인되는 경우에만 사건별 근거와 함께 표시합니다. 예상 보험금·보험료·PML은 이 화면에서 산출하지 않습니다.</div></section>
