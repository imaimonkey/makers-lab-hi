...
- 약관화 검토의 보장 범위·보장 제외 범위 카드는 같은 데스크톱 행에서 가장 긴 내용 기준으로 높이를 맞춰 비교가 흔들리지 않도록 했다.
- 상품개발제안 1차 가격·손해지표는 카드별 추가 산출 근거 문구가 있어도 같은 행의 카드 높이를 공유하도록 정렬했다.
- 상품개발제안의 `추가 근거 상세 보기`는 데스크톱 2열에서 근거 카드 네 개가 같은 높이를 사용하도록 정렬했다.
- 상품개발제안의 `산출 근거와 신뢰도`는 안쪽 목록을 세로 카드로 재구성하고, 바깥 3개 근거 그룹을 넓은 보드 안에서 읽히도록 정리했다.
- 상품개발제안 1차 가격·손해지표의 산출식 패널은 두 번째 줄 카드의 고정 높이와 자동 상단 여백을 제거해 열릴 때 아래 방향으로 자연스럽게 확장되도록 했다. 섹션 설명 문구는 제거했다.
- 상품개발제안의 우선 추천 이유 카드는 번호와 제목을 같은 헤더 행으로 정렬하고, 카드 높이를 콘텐츠 기준으로 줄여 불필요한 빈 공간을 덜어냈다.
- 상품개발제안 보험금 지급 흐름 하단의 보조 안내 문구는 화면 밀도를 위해 제거하고, 지급 단계 데이터와 흐름은 유지했다.
- 상품개발제안의 `산출 근거와 신뢰도`는 안쪽 개별 박스를 얇은 행 구분으로 바꾸고, 주황색 변수 강조를 회청색으로 통일해 정보 밀도를 낮췄다.
...
- 종합브리핑 요약 보드는 중첩된 외곽선의 강도를 단계별로 낮추고, 결론·핵심 판단·정량 지표 카드는 얕은 그림자로 구획을 유지하도록 정리했다.
- 종합브리핑의 `확인된 판단`·`후속 확인` 카드와 핵심 정량 지표 4개 카드는 테두리를 제거하고 카드 그림자로 구획을 표시하도록 조정했다.
- 종합브리핑 핵심 정량 지표 4개 카드에는 그림자를 유지하면서도 카드 경계가 보이도록 연한 외곽선을 다시 적용했다.
- 종합브리핑 상단 요약 영역의 중복 `SUMMARY`·`검토 결론 요약` 헤더를 제거하고 결론·핵심 판단·정량 지표 사이의 여백을 압축했다.
- 종합브리핑의 `RISK OVERVIEW`를 `CORE REVIEW RESULTS`보다 앞에 배치해 위험 설명 후 핵심 검토 결과를 읽는 순서로 정리했다.
- 상품화 검토 종합평가의 중복 안내 문구를 제거하고, 제목 아래에서 상품화 판정 카드가 바로 시작되도록 상단 여백을 줄였다.
- 약관검토의 보험금 지급조건 영역을 아이콘 카드형 헤더에서 영문 보조 제목과 한글 제목을 사용하는 텍스트형 섹션 헤더로 정리했다.
- 상품개발제안 보장 구조 요약 카드의 분류 배지와 카드 제목을 한 줄로 정렬하고, 절약된 공간만큼 카드 내부 여백과 높이를 줄였다.
- 종합브리핑 핵심 판단 근거 카드의 번호 배지와 영역 제목을 하나의 묶음으로 중앙 정렬했다.
- 종합브리핑 `RISK OVERVIEW`의 보조 제목·섹션 제목·위험 맥락 본문 크기를 핵심 검토 결과와 균형이 맞도록 조정했다.
- 약관화 검토의 `약관 작성 근거와 핵심 용어`는 같은 높이의 2개 카드로 정리하고, 핵심 용어는 대표 6개만 먼저 보여준 뒤 나머지를 `핵심 용어 더보기` 토글로 확인하도록 구성했다.
...
## 2026-08-10 전기차 위험 심층 해설 원문·AI 해설 연결

- `/risks/ev-battery-fire` 심층 해설의 각 항목을 원문 줄글, 문장 끝 출처 링크 뱃지, 다음 줄의 `AI 해설` 카드 순서로 표시한다.
- 연결된 원문이 있는 항목만 출처 링크 뱃지를 표시하고, 원문이 없는 항목의 `원문 확인 필요` 태그는 표시하지 않는다. 심층 해설 원문과 AI 해설 카드의 핵심 위험·손해·책임·상품화 조건 문구는 굵게 표시한다.
- 외부 API·저장 계약과 안정 경로는 변경하지 않았고, 운영 전에는 원문 발췌·수집 시각·근거 ID를 실제 근거 원장 계약에 연결해야 한다.
- `npm run check` 통과.
