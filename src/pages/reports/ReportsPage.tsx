...
import { createGasReportProxy } from '../../report/api/gas-report-proxy'
import '../../report/report.css'

const { riskData, fallbackReport } = getMockReportData()

const reportProxy = createGasReportProxy('/api/report-assistant')
  ...
  return (
    <div className="page reports-page">
      ...
      <section className="report-list surface-card">
        <div className="panel-heading"><div><p className="eyebrow">REPORT LIBRARY</p><h2>최근 의사결정 문서</h2></div><button type="button" className="primary-small" onClick={openIntegratedReport}>+ 새 리포트 생성</button></div>
        {reports.map((report) => (
          <article className="report-row" key={report.title}>
            ...
            <button type="button" aria-label={`${report.title} 통합 리포트 워크벤치로 이동`} onClick={openIntegratedReport}><AppIcon name="arrow" size={17} /></button>
          </article>
        ))}
      </section>
      ...
      <section id="report-generation-slice" className="report-generation-slice surface-card" aria-label="리포트 생성·검토 데모">
        <div className="report-generation-slice-heading">
          ...
          <span className="status-badge">SAMPLE FALLBACK · SERVER PROXY READY</span>
        </div>
        <ReportPage
          riskData={riskData}
