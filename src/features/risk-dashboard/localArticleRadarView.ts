...
export function buildLocalArticleRadarView(records: ArticleSourceRecord[]): LocalArticleRadarView {
  ...
  const priorities = ordered.slice(0, 5).map(priority)
  const first = ordered[0]
  const firstScores = first ? scores(first) : { market: 0, pml: 0, productization: 0 }
  const firstPriority = priorities[0]
  const candidates = ordered.slice(0, 12).map((article) => ({ id: article.id, detailRiskId: `developer-${article.id}`, title: article.title, description: `[${article.contentProfile.topic}] ${article.summary}`, tags: [article.contentProfile.topic, ...article.contentProfile.keywords.slice(0, 2)], keywords: [article.contentProfile.topic, ...article.contentProfile.keywords] }))
  ...
  const marketUpdates = ordered.slice(0, 8).map((article) => ({ id: `update-${article.id}`, type: article.derived.isRegulatory ? '법령·규제 자료' : '보험·재보험 자료', source: article.source ?? '문서 원문', date: dateLabel(article.collectedAt), displayDate: dateLabel(article.collectedAt), title: article.title, insight: article.summary ?? article.contentProfile.summary, relatedTopic: article.contentProfile.topic, sourceUrl: article.fileUrl || null }))
  const globalInsights = ordered.filter((article) => !article.derived.isRegulatory).slice(0, 6).map((article) => ({ id: `global-${article.id}`, organization: article.source ?? '문서 원문', date: dateLabel(article.collectedAt), displayDate: dateLabel(article.collectedAt), title: article.title, summary: article.summary ?? article.contentProfile.summary, relatedRisk: article.contentProfile.topic, sourceType: '문서 원문', sourceTitle: article.title, sourceUrl: article.fileUrl || null }))
  const recentProducts = ordered.filter((article) => /보험시장|반려동물|건강보험|생명보험|재보험/.test(article.contentProfile.topic)).slice(0, 5).map((article) => ({ id: `product-${article.id}`, company: article.source ?? '문서 원문', title: article.title, summary: article.summary ?? article.contentProfile.summary, date: dateLabel(article.collectedAt), sourceType: '시장 자료', sourceUrl: article.fileUrl || null }))
  const exclusiveRights: ExclusiveRight[] = []
  const scraps = ordered.slice(0, 5).map((article) => ({ id: `scrap-${article.id}`, title: article.title, conclusion: `시장성 ${scores(article).market.toFixed(1)}점 · PML ${scores(article).pml.toFixed(1)}점 · 상품화 종합점수 ${scores(article).productization.toFixed(1)}점`, reportId: `article-report-${article.id}` }))
  ...
  const sourceShares: RiskRadarSourceShare[] = [...sourceCounts.entries()].map(([label, count], index) => ({ label, share: Math.round(count / total * 100), role: label === '법령·규제 자료' ? '적용 범위·책임 주체·보상 기준' : label === '보험시장·기업자료' ? '보험료·자본력·시장 흐름' : '위험 신호·시장·손해 구조', tone: index === 0 ? 'orange' : index === 1 ? 'navy' : index === 2 ? 'blue' : 'mist' }))
  if (sourceShares.length) sourceShares[sourceShares.length - 1].share += 100 - sourceShares.reduce((sum, item) => sum + item.share, 0)

  return {
    kpis: [{ label: '신규 위험 후보', value: records.length, unit: '건', description: '연결된 문서에서 분류·분석한 위험 후보', meta: '연결 문서 기준', icon: 'radar', tone: 'orange' }, { label: '상품화 검토 후보', value: Math.min(ordered.length, 8), unit: '건', description: '시장성·PML·근거 수준을 함께 계산한 후보', meta: '종합점수 기준', icon: 'report', tone: 'navy' }, { label: '법·규제 연계 위험', value: legalArticles.length, unit: '건', description: '상품화 후보에 적용 기준과 책임 주체를 보강하는 법령 자료', meta: '법령 자료 기준', icon: 'shield', tone: 'blue' }],
